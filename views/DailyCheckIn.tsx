...
export const DailyCheckIn: React.FC<Props> = ({
  state,
  updateState,
  onComplete,
}) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="flex justify-between">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full border border-primary/10 transition-all ${
                    formData.mood === n
                      ? "bg-primary text-glass-surface"
        ...
        <div className="mb-6">
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                deepWorkMinutes: parseInt(e.target.value) || 0,
              })
            }
          />
        </div>
        <div className="grid grid-cols-2 gap-4">
          {Object.entries(formData.reps)
            .filter(([key]) => key !== "freedive")
            .map(([key, val]) => (
              <button
                key={key}
                type="button"
                onClick={() =>
                  setFormData((prev) => ({
                    ...prev,
                    reps: { ...prev.reps, [key]: !val },
                  }))
                }
                className={`p-4 rounded-lg border text-left transition-all ${
                  val
                    ? "bg-emerald-500/20 border-emerald-500/30 text-emerald-500 dark:text-emerald-100"
      ...
      <GlassCard title="Habits (Identity)">
        <div className="space-y-2">
          {state.habits.filter((h) => h.active).length === 0 && (
            <p className="text-sm text-muted">No active habits configured.</p>
          )}
          {state.habits
            .filter((h) => h.active)
            .map((habit) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded cursor-pointer transition-colors ${
                    isDone ? "bg-primary/10" : "hover:bg-primary/5"
                  }`}
                >
                  <div
                    className={`w-5 h-5 rounded border border-primary/30 flex items-center justify-center ${
                      isDone ? "bg-primary text-glass-surface" : ""
                    }`}
                  >
                    {isDone && <Save size={12} />}
                  </div>
                  <span className="text-primary">{habit.name}</span>
                  <span className="text-xs uppercase text-muted ml-auto">
                    {habit.category}
                  </span>
                </div>
              );
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  reflection: { ...formData.reflection, win: e.target.value },
                })
              }
            />
          </div>
      ...
      <button
        type="submit"
        className="w-full bg-primary text-glass-surface font-bold py-4 rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-primary/10"
      >
        Commit Log
      </button>
    </form>
