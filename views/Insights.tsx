...
export const Insights: React.FC<Props> = ({ state, updateState }) => {
  ...
  const checkCriteria = (ruleId: string) => {
    const rule = Object.values(VISUALIZATION_RULES).find(r => r.id === ruleId);
    if (!rule) return { eligible: false, remainingDays: 0, reflectionDensity: 0 };
  ...
  const handleUnlock = (ruleId: string) => {
    if (justification.length < 10) return alert("Please justify why you need this metric.");
    updateState(prev => ({
      ...prev,
  ...
  const renderLockedCard = (rule: any) => {
    ...
    return (
      <GlassCard key={rule.id} className="relative group overflow-hidden" hoverEffect={true}>
        <div className="absolute inset-0 bg-primary/5 backdrop-blur-sm z-10 flex flex-col items-center justify-center text-center p-6">
          {status.eligible ? (
            <div className="space-y-4 max-w-sm animate-fade-in">
              ...
              <h3 className="text-xl font-medium text-primary">Unlock Available</h3>
              <p className="text-sm text-muted">You have gathered enough signal. One last step.</p>
              
              {selectedUnlockId === rule.id ? (
                <div className="mt-4 w-full">
                  <p className="text-xs text-left mb-2 text-primary/60">Decision Test: What decision does this help you make better?</p>
                  <input 
            ...
            <div className="space-y-2">
              ...
              <h3 className="text-lg font-medium text-primary/50">{rule.title} Locked</h3>
              <div className="text-xs text-left bg-primary/5 p-4 rounded mt-4 space-y-2 w-full max-w-xs border border-primary/5">
                <div className="flex justify-between">
                  ...
                  <span className={status.remainingDays === 0 ? "text-emerald-500" : "text-primary"}>
                    {state.logs.length}/{rule.daysRequired}
                  </span>
                </div>
                <div className="w-full bg-primary/5 h-1 rounded overflow-hidden">
                  <div className="h-full bg-primary/20" style={{ width: `${(state.logs.length / rule.daysRequired) * 100}%` }} />
                </div>
                
                <div className="flex justify-between pt-2">
                  <span className="text-muted">Reflection Density</span>
                  <span className={status.reflectionDensity >= 70 ? "text-emerald-500" : "text-primary"}>
                    {Math.round(status.reflectionDensity)}% / 70%
                  </span>
                </div>
                <div className="w-full bg-primary/5 h-1 rounded overflow-hidden">
                  <div className="h-full bg-primary/20" style={{ width: `${Math.min(100, status.reflectionDensity)}%` }} />
                </div>
              </div>
              <p className="text-xs text-muted mt-2 italic">"More reflection needed before trends emerge."</p>
            </div>
          )}
        </div>
  ...
  const weeklyData = React.useMemo(() => {
    const weeks: Record<string, { name: string, deepWork: number, presence: number }> = {};
  ...
  return (
    <div className="space-y-8">
      <GlassCard className="mb-8">
        <div className="flex items-start gap-4">
          ...
          <p className="text-sm text-muted">
            Visualizations are tools for specific questions. They are hidden by default to prevent you from optimizing for the graph instead of the reality.
          </p>
        </div>
      </GlassCard>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        {/* --- Weekly Aggregates --- */}
        {state.unlockedVisualizations.includes(VISUALIZATION_RULES.WEEKLY_AGGREGATES.id) ? (
          <GlassCard title="Weekly Deep Work & Presence" hoverEffect={true}>
            <div className="h-64 w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyData}>
                  <XAxis dataKey="name" stroke="var(--chart-stroke)" fontSize={10} tickLine={false} />
                  <YAxis stroke="var(--chart-stroke)" fontSize={10} tickLine={false} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: 'rgba(var(--text-base), 0.9)', borderColor: 'rgba(var(--text-base), 0.1)', color: 'rgba(var(--glass-bg-rgb), 1)' }}
                    itemStyle={{ color: 'rgba(var(--glass-bg-rgb), 1)' }}
                  />
                  <Bar dataKey="deepWork" fill="#a58e7e" name="Deep Work (m)" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="presence" fill="#7ea586" name="Presence Reps" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <p className="text-xs text-muted mt-4 text-center">Are you compounding focus or fragmentation?</p>
          </GlassCard>
        ) : renderLockedCard(VISUALIZATION_RULES.WEEKLY_AGGREGATES)}


        {/* --- Habit Heatmap --- */}
        {state.unlockedVisualizations.includes(VISUALIZATION_RULES.HABIT_HEATMAP.id) ? (
          <GlassCard title="Identity Consistency (Last 30 Days)" hoverEffect={true}>
               ...
               {last30Days.map(day => (
                 <div 
                   ...
                   className={`w-8 h-8 rounded-sm transition-all hover:scale-110 ${
                     day.count === 0 ? 'bg-primary/5' : 
                     day.count <= 2 ? 'bg-emerald-900/40 dark:bg-emerald-900/40' : 
                     day.count <= 4 ? 'bg-emerald-600/60' : 'bg-emerald-400/80'
                   }`}
                 />
               ))}
            </div>
        ...
        {/* --- Energy vs Output --- */}
        {state.unlockedVisualizations.includes(VISUALIZATION_RULES.ENERGY_OUTPUT.id) ? (
          <GlassCard title="Energy vs. Output Correlation" hoverEffect={true}>
            <div className="h-64 w-full mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  <XAxis type="number" dataKey="x" name="Energy" unit="" domain={[0, 6]} stroke="var(--chart-stroke)" tickCount={6} />
                  <YAxis type="number" dataKey="y" name="Deep Work" unit="m" stroke="var(--chart-stroke)" />
                  <Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: 'rgba(var(--text-base), 0.9)', border: 'none', color: 'rgba(var(--glass-bg-rgb), 1)' }} />
                  <Scatter name="Days" data={scatterData} fill="#7E9aa5" />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
            <p className="text-xs text-muted mt-4 text-center">Do you need high energy to produce, or can you ship when tired?</p>
          </GlassCard>
