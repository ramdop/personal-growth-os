...
const getAppStateDecl: FunctionDeclaration = {
  name: 'getAppState',
  description: 'Get the current state of the user app, including logs, habits, and objectives.',
  parameters: { type: Type.OBJECT, properties: {} }
};

const logDailyCheckInDecl: FunctionDeclaration = {
  name: 'logDailyCheckIn',
  description: 'Log or update the daily check-in. Partial updates are allowed (e.g., just logging mood).',
  parameters: {
    type: Type.OBJECT,
    properties: {
      mood: { type: Type.NUMBER, description: 'Mood 1-5' },
      energy: { type: Type.NUMBER, description: 'Energy 1-5' },
      deepWorkMinutes: { type: Type.NUMBER, description: 'Minutes of deep work' },
      presenceRep: { type: Type.BOOLEAN },
      buildRep: { type: Type.BOOLEAN },
      workoutRep: { type: Type.BOOLEAN },
      freediveRep: { type: Type.BOOLEAN },
...
const addHabitDecl: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      ...
      category: { type: Type.STRING, description: 'Category: presence, builder, health, mind' }
    },
...
const updateHabitDecl: FunctionDeclaration = {
  name: 'updateHabit',
  description: 'Update an existing habit (rename, change category, or archive) by searching for its name.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      targetName: { type: Type.STRING, description: 'The current name of the habit to update (fuzzy match)' },
      newName: { type: Type.STRING, description: 'The new name (optional)' },
      newCategory: { type: Type.STRING, description: 'The new category (optional)' },
      active: { type: Type.BOOLEAN, description: 'Set to false to archive/deactivate (optional)' }
    },
    required: ['targetName']
  }
};
...
const removeHabitDecl: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    properties: {
      name: { type: Type.STRING, description: 'The name of the habit to remove (fuzzy match)' }
    },
    required: ['name']
  }
};
...
const updateObjectiveDecl: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    properties: {
      targetTitle: { type: Type.STRING, description: 'Current title (fuzzy match)' },
...
const removeObjectiveDecl: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    properties: {
      title: { type: Type.STRING, description: 'Title of the objective to remove (fuzzy match)' }
    },
    required: ['title']
  }
};
...
const addKeyResultDecl: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    properties: {
      objectiveTitle: { type: Type.STRING, description: 'The exact title of the objective to match.' },
      title: { type: Type.STRING, description: 'Key Result title' },
      target: { type: Type.STRING, description: 'Target metric or qualitative state' }
    },
...
const updateKeyResultDecl: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    properties: {
      objectiveTitle: { type: Type.STRING, description: 'The title of the objective containing the KR (fuzzy match)' },
      krTitle: { type: Type.STRING, description: 'The current title of the KR (fuzzy match)' },
      newTitle: { type: Type.STRING },
      newTarget: { type: Type.STRING },
      newCurrent: { type: Type.STRING, description: 'Current progress value (e.g., "5/10 books")' },
...
const removeKeyResultDecl: FunctionDeclaration = {
  ...
  parameters: {
    ...
    properties: {
      objectiveTitle: { type: Type.STRING, description: 'The title of the objective containing the KR (fuzzy match).' },
      keyResultTitle: { type: Type.STRING, description: 'The title of the Key Result to remove (fuzzy match).' }
    },
...
export const AICompanion: React.FC<AICompanionProps> = ({ state, updateState }) => {
  ...
  const suggestions = useMemo(() => {
    const isColdStart = state.habits.length === 0 && state.objectives.length === 0;

    if (isColdStart) {
      return [
        { label: "Discover Habits", text: "Help me design habits based on the identity I want to build.", icon: "🌱", desc: "Setup your system" },
        { label: "Define Vision", text: "Help me draft my annual objectives and themes.", icon: "🔭", desc: "Set your direction" },
        { label: "Explain Logic", text: "What is the philosophy behind this system?", icon: "🧠", desc: "Understand the OS" },
        { label: "Daily Check-in", text: "Start daily check-in", icon: "📝", desc: "Log mood & reps" },
      ];
    }

    return [
      { label: "Daily Check-in", text: "Start daily check-in", icon: "📝", desc: "Log mood & reps" },
      { label: "Analyze Signal", text: "How am I doing based on my logs?", icon: "📊", desc: "Data-driven insights" },
      { label: "Add Habit", text: "I want to add a new habit", icon: "✨", desc: "Track a new behavior" },
      { label: "Set Objective", text: "Help me set a new objective", icon: "🎯", desc: "Define annual goals" },
    ];
    ...
    const initChat = async () => {
      try {
        ...
        setMessages(prev => {
          ...
          return [...prev, { id: Date.now().toString(), role: 'system', text: "System instructions updated." }];
        });
  ...
  const executeTool = async (call: any) => {
    ...
    try {
      ...
      if (name === 'logDailyCheckIn') {
        ...
        updateState(prev => {
          ...
          const newLog = {
            ...
            deepWorkMinutes: args.deepWorkMinutes ?? existing?.deepWorkMinutes ?? 0,
            reps: {
              ...
              freedive: args.freediveRep ?? existing?.reps.freedive ?? false,
            },
            reflection: {
          ...
          return { ...prev, logs: [...prev.logs.filter(l => l.date !== today), newLog] };
        });
      ...
      if (name === 'updateHabit') {
        const targetHabit = state.habits.find(h => h.name.toLowerCase().includes(args.targetName.toLowerCase()));
        ...
        updateState(prev => ({
          ...prev,
          habits: prev.habits.map(h => h.id !== targetHabit.id ? h : {
            ...h,
        ...
        return { status: 'success', message: `Habit "${targetHabit.name}" updated.` };
      }

      if (name === 'removeHabit') {
        const targetHabit = state.habits.find(h => h.name.toLowerCase().includes(args.name.toLowerCase()));
        if (!targetHabit) {
        ...
        return { status: 'success', message: `Habit "${targetHabit.name}" removed.` };
      }
      ...
      if (name === 'updateObjective') {
        const targetObj = state.objectives.find(o => o.title.toLowerCase().includes(args.targetTitle.toLowerCase()));
        if (!targetObj) return { error: `Objective "${args.targetTitle}" not found.` };

        updateState(prev => ({
          ...prev,
          objectives: prev.objectives.map(o => o.id !== targetObj.id ? o : {
            ...o,
      ...
      if (name === 'removeObjective') {
        const targetObj = state.objectives.find(o => o.title.toLowerCase().includes(args.title.toLowerCase()));
        if (!targetObj) return { error: `Objective "${args.title}" not found.` };

        updateState(prev => ({
          ...prev,
          objectives: prev.objectives.filter(o => o.id !== targetObj.id)
        }));
        return { status: 'success', message: `Objective "${targetObj.title}" removed.` };
      }

      if (name === 'addKeyResult') {
        const targetObj = state.objectives.find(o => o.title.toLowerCase().includes(args.objectiveTitle.toLowerCase()));

        if (!targetObj) {
          return { error: `Objective containing "${args.objectiveTitle}" not found.` };
        }
      ...
      if (name === 'updateKeyResult') {
        ...
        updateState(prev => {
          const targetObj = prev.objectives.find(o => o.title.toLowerCase().includes(args.objectiveTitle.toLowerCase()));
          if (!targetObj) return prev;
          foundObj = true;

          const targetKr = targetObj.keyResults.find(k => k.title.toLowerCase().includes(args.krTitle.toLowerCase()));
          ...
          return {
            ...
            objectives: prev.objectives.map(o => o.id !== targetObj.id ? o : {
              ...o,
              keyResults: o.keyResults.map(k => k.id !== targetKr.id ? k : {
                ...k,
        ...
        if (!foundObj) return { error: `Objective "${args.objectiveTitle}" not found.` };
        if (!foundKr) return { error: `Key Result "${args.krTitle}" not found.` };
      ...
      if (name === 'removeKeyResult') {
        ...
        updateState(prev => {
          ...
          const targetKr = targetObj.keyResults.find(k => k.title.toLowerCase().includes(args.keyResultTitle.toLowerCase()));
          if (!targetKr) return prev;
          foundKr = true;
          ...
          return {
            ...
            objectives: prev.objectives.map(o => o.id !== targetObj.id ? o : {
              ...o,
              keyResults: o.keyResults.filter(k => k.id !== targetKr.id)
            })
          };
        });

        if (!foundObj) return { error: `Objective similar to "${args.objectiveTitle}" not found.` };
        if (!foundKr) return { error: `Key Result similar to "${args.keyResultTitle}" not found in objective "${args.objectiveTitle}".` };
  ...
  const processModelResponse = async (response: any) => {
    ...
    if (response.functionCalls && response.functionCalls.length > 0) {
      ...
      const safeResults = await Promise.all(safeCalls.map(async (call: any) => {
      ...
      if (unsafeCalls.length > 0) {
        ...
        const nextResponse = await chatRef.current.sendMessage({ message: safeResults });
  ...
  const handleActionDecision = async (message: Message, approved: boolean) => {
    ...
    setMessages(prev => prev.map(m => m.id === message.id ? { ...m, actionStatus: approved ? 'approved' : 'denied' } : m));
    ...
    try {
      const nextResponse = await chatRef.current.sendMessage({ message: allResults });
      await processModelResponse(nextResponse);
    } catch (e) {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', text: "Error: Connection interrupted." }]);
      setIsProcessing(false);
    }
  };
      ...
      case 'addHabit':
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm">"{args.name}" <span className="text-xs text-muted">({args.category})</span></span>
          </div>
        );
      case 'updateHabit':
        return (
          <div className="flex flex-col gap-1">
            ...
            {args.newName && <span className="text-xs text-muted">New Name: {args.newName}</span>}
            {args.newCategory && <span className="text-xs text-muted">New Cat: {args.newCategory}</span>}
            {args.active !== undefined && <span className="text-xs text-muted">Active: {args.active.toString()}</span>}
          </div>
        );
      case 'removeHabit':
        return (
      ...
      case 'updateObjective':
        return (
          <div className="flex flex-col gap-1">
            ...
            {args.newTitle && <span className="text-xs text-muted">New Title: {args.newTitle}</span>}
          </div>
        );
      ...
      case 'addKeyResult':
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm">{args.title}: {args.target}</span>
            <span className="text-xs text-muted">To: "{args.objectiveTitle}"</span>
          </div>
        );
      case 'updateKeyResult':
        return (
          <div className="flex flex-col gap-1">
            <span className="font-medium text-amber-400">Update Key Result</span>
            <span className="text-sm">"{args.krTitle}"</span>
            <span className="text-xs text-muted">Obj: "{args.objectiveTitle}"</span>
            {args.newCurrent && <span className="text-xs text-emerald-300">New Progress: {args.newCurrent}</span>}
          </div>
        );
      case 'removeKeyResult':
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-muted">From: "{args.objectiveTitle}"</span>
          </div>
        );
        ...
        return (
          <div className="flex flex-col gap-1">
            ...
            <div className="text-xs text-muted grid grid-cols-2 gap-1">
              ...
              {args.deepWorkMinutes && <span>Deep Work: {args.deepWorkMinutes}m</span>}
        ...
        <div className="fixed bottom-40 right-6 md:bottom-32 md:right-12 w-[90vw] md:w-[450px] h-[75vh] z-50 animate-fade-in origin-bottom-right">
          ...
          <div className="glass-panel h-full flex flex-col rounded-2xl overflow-hidden shadow-2xl border-primary/20">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
                ...
                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  {msg.text && (
                    <div
                      ...
                      `}
                    >
                      {msg.role === 'system' && <Terminal size={10} className="inline mr-2" />}
                      {msg.role === 'model' ? (
                  ...
                  {msg.isActionRequired && msg.toolCalls && (
                    <div className="mt-2 w-[90%] bg-glass-surface border border-primary/10 rounded-xl p-4 shadow-lg animate-fade-in ring-1 ring-primary/5">
                      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-primary/5">
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider text-primary">Proposed Actions</span>
                      </div>

                      <div className="space-y-3 mb-4 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                        {msg.toolCalls.map((call, idx) => (
                          <div key={idx} className="bg-primary/5 rounded p-3 text-sm border border-primary/5">
                      ...
                      {msg.actionStatus === 'pending' ? (
                        ...
                        <div className={`text-center text-xs font-bold uppercase tracking-wide py-2 rounded ${msg.actionStatus === 'approved' ? 'text-emerald-500 bg-emerald-500/5' : 'text-red-400 bg-red-500/5'}`}>
                  ...
                  {msg.suggestions && index === messages.length - 1 && !isProcessing && (
              ...
              {messages.filter(m => m.role === 'user').length === 0 && !isProcessing && (
                <div className="grid grid-cols-2 gap-3 mt-4 animate-fade-in px-1">
                  {suggestions.map((s, i) => (
                    ...
                    >
                      <div className="text-xl mb-2 group-hover:scale-110 transition-transform origin-left">{s.icon}</div>
                      <div className="font-medium text-sm text-primary leading-tight">{s.label}</div>
                      <div className="text-[10px] text-muted mt-1 leading-tight">{s.desc}</div>
                    </button>
                  ))}
                </div>
              )}
